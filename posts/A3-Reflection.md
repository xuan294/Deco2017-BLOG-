...
I focused on semantic HTML to assure basic accessibility. The application is fully keyboard navigable using the appropriate `<form>`, `<label>`, and `<button>` tags. Users can navigate, filter, and fill out forms entirely without a mouse. I also ensured strong color contrast between text and backgrounds — for example, using dark gray text on a soft off-white background — to meet WCAG AA standards, which is reflected in the Lighthouse Accessibility score of 95.
